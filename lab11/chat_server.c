...
int accept_connection(int fd, struct sockname *users) {
	...
	char username_input[BUF_SIZE];
	int num_read = read(client_fd, &username_input, BUF_SIZE);
	if (num_read == -1){
		exit(0);
	}
...
int read_from(int client_index, struct sockname *users) {
     ...
     */
    int num_read = read(fd, &buffer, BUF_SIZE);
    buffer[num_read] = '\0';
	...
	strncat(msg, buffer, BUF_SIZE - strlen(msg));
