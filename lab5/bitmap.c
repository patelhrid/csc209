...
//	image = fopen(*image, "rb");
	...
	int sz = ftell(image);
	*pixel_array_offset = sz - 3 * *width * *height;
//	fclose(*image);
}
...
//	fopen(*image, "rb");
	...
	for (int a = 0; a < height; a++){
		for (int b = 0; b < width; b++){
			...
			fread(&curr_pixel.blue, 1, 1, image);
			fread(&curr_pixel.green, 1, 1, image);
			fread(&curr_pixel.red, 1, 1, image);
			pixels[a][b] = curr_pixel;
		}
	}
//	fclose(*image);
	return pixels;
